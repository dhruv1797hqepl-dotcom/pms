...
const EmployeeDashboard = () => {
  ...
  try {
    ...
    const dashboardUrl = hasValidMemberId
      ? `tasks/employee-dashboard/?member_id=${memberId}`
      : `tasks/employee-dashboard/`;

    const [dashboardRes, projectsRes, directoryInternalRes, directoryExternalRes, clientsRes] = await Promise.all([
    ...
    const allFetchedTasks = dashboardRes.data.tasks;
    const isMemberView = hasValidMemberId;

    setUserName(getDashboardDisplayName(userData));
    setCurrentUser(userData);

    // Set directory
    setAssignableDirectory({
      internal: normalizeListResponse(directoryInternalRes.data).map(buildMemberFromUser).filter(Boolean),
      externalClient: normalizeListResponse(directoryExternalRes.data).map(buildMemberFromUser).filter(Boolean),
    });

    // Build project map
    const projectsData = normalizeListResponse(projectsRes.data);
    const mapping = {};
