...
const MCTC = () => {
    ...
    const generateTaskPDF = () => {
        ...
        weeks.forEach((week, weekIndex) => {
            ...
            week.forEach((cell, dayIndex) => {
                ...
                const dayEntries = (tasks[cell.key] || []).filter(e => !isPlaceEntry(e));
                const half1 = dayEntries.filter(t => !t.half_type || t.half_type === "first_half");
                const half2 = dayEntries.filter(t => t.half_type === "second_half");
                const maxTextWidth = colWidth - 6;
                ...
                if (half1.length > 0) {
                    half1.slice(0, 2).forEach((t) => {
                        const completed = isLinkedTaskCompleted(t);
                        doc.setFont("helvetica", "normal");
                        doc.setFontSize(6.5);
                        doc.setTextColor(...(completed ? [4, 120, 87] : [146, 64, 14]));
                        doc.text(truncateText(formatCalendarTaskLabel(t.label), maxTextWidth, 6.5), textX, cursorY);
                        cursorY += 3.5;
                    });
                    if (half1.length > 2) {
                        doc.setFontSize(5.5);
                        doc.setTextColor(148, 163, 184);
                        doc.text(`+${half1.length - 2} more`, textX, cursorY);
                        cursorY += 3.5;
                    }
                } else {
                    doc.setFont("helvetica", "normal");
                ...
                if (half2.length > 0) {
                    ...
                    if (half2.length > 2) {
                        doc.setFontSize(5.5);
                        doc.setTextColor(148, 163, 184);
                        doc.text(`+${half2.length - 2} more`, textX, cursorY);
                    }
                } else {
                    doc.setFont("helvetica", "normal");
                    doc.setFontSize(6.5);
    ...
    const downloadDayTaskPdf = (dayKey) => {
        if (!dayKey) return;
        const dayTaskList = (tasks[dayKey] || []).filter((entry) => !isPlaceEntry(entry));
        ...
        doc.setFont("helvetica", "bold");
        doc.setTextColor(30, 41, 59);
        doc.text("MCTC Task Report", margin, 18);

        doc.setFontSize(11);
        doc.setFont("helvetica", "normal");
        doc.setTextColor(100);
        doc.text(formatDayLabel(dayKey), margin, 26);

        if (isMemberView) {
            doc.setFontSize(10);
            doc.text(`Employee: ${targetUserLabel}`, margin, 33);
        }

        doc.setTextColor(0);

        const half1Tasks = dayTaskList.filter(t => !t.half_type || t.half_type === "first_half");
        const half2Tasks = dayTaskList.filter(t => t.half_type === "second_half");

        const renderHalfTable = (halfTasks, halfLabel, startY) => {
            doc.setFontSize(10);
            doc.setFont("helvetica", "bold");
            doc.setTextColor(30, 41, 59);
            doc.text(halfLabel, margin, startY);

            autoTable(doc, {
                startY: startY + 3,
                margin: { left: margin, right: margin },
                head: [["#", "Task", "Type", "Status"]],
                body: halfTasks.length > 0
                    ? halfTasks.map((t, i) => [
                        i + 1,
                        formatCalendarTaskLabel(t.label),
                        t.type || "task",
                        isLinkedTaskCompleted(t) ? "Done" : "Pending",
                    ])
                    : [["", "No tasks for this half", "", ""]],
        ...
        let cursorY = isMemberView ? 40 : 34;
        cursorY = renderHalfTable(half1Tasks, "HALF 1", cursorY);
        renderHalfTable(half2Tasks, "HALF 2", cursorY);
        ...
        doc.save(`MCTC_Tasks_${dayKey}.pdf`);
    };
